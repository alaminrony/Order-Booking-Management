...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>@lang('lang.DEPARTMENT_VS_USERS')</h1>
                </div>
                <div class="col-sm-6">
                    <div class="float-right mr-2">
                        <a type="button" class="btn btn-success openCreateModal" data-toggle="modal" title="@lang('lang.CREATE_DEPARTMENT_VS_USERS')" data-target="#viewCreateModal"><i class="fa fa-plus-square"></i> @lang('lang.CREATE_DEPARTMENT_VS_USERS')</a>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-12">
                    <div class="card card-secondary">
                        <div class="card-header">
                            <h3 class="card-title">@lang('lang.DEPARTMENT_VS_USER_LIST')</h3>
                        </div>
                        ...
                        <div class="card-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$i}}</td>
                                        <td>{{$departments[$target->department_id]?? ''}}</td>
                                        <td>{{$users[$target->user_id]?? ''}}</td>
                                        <td>{{$users_email[$target->user_id]?? ''}}</td>
                                        ...
                                        <td>
                                            ...
                                            <div style="float: left;">
                                                {!!Form::open(['route'=>['departmentVsUsers.destroy',$target->id]])!!}
                                                @method('DELETE')
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.openCreateModal', function () {
            $.ajax({
                url: "{{route('departmentVsUsers.create')}}",
                type: "post",
                dataType: "json",
                headers: {
        ...
        $(document).on('click', '#create', function () {
            ...
            if (data != '') {
                $.ajax({
                    url: "{{route('departmentVsUsers.store')}}",
                    data: data,
                    cache: false,
                    contentType: false,
                    processData: false,
